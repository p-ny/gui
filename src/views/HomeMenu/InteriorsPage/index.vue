<template>
  <div class="interiors-page">
    <Head :titleText="loc('home:interior:head')"/>
    <div class="header">
      <div class="header__info">
        <div class="info-item">
          <div class="value">{{interiorList.length}}</div>
          <div class="desc">{{loc('HomeMenu_9')}}</div>
        </div>
        <div class="info-item">
          <div class="value">{{interiorsBought}}</div>
          <div class="desc">{{loc('HomeMenu_10')}}</div>
        </div>
      </div>
      <div class="header__nav">
        <div class="header__nav-item" @click="setCurrentPage('FurniturePage')">{{loc('HomeMenu_11')}}</div>
      </div>
    </div>
    <div class="scroll">
      <div class="body">
        <div
          ...
          :key="item.id"
          @click="watchInterior(item.name, item.key)"
        >
          <div class="desc">{{item.name}}</div>
          <div class="title">#{{item.key}}</div>
          <div class="btn btn-watch"></div>
          <div class="img" :style="{backgroundImage: 'url(/img/homeMenu/items/interior-' + item.key + '.png)'}"></div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...mapState('homeMenu', ['interiorList', 'houseId']),
    ...mapGetters('localization', ['loc']),
    interiorsBought: function() {
      return this.interiorList.filter(item => item.bought).length
    }
  },

  methods: {
    ...mapMutations('homeMenu', ['setCurrentPage']),
    watchInterior: function(name, key) {
      window.mp.trigger('homeMenu:watchInterior', this.houseId, name, key)
    }
  }
}
</script>
...
.interiors-page{
  ...
  .header{
    ...
    &__nav{
      &-item{
        &:before{
          background-image: url('/img/homeMenu/icon-furniture.svg');
        }
      }
    }
  }
    ...
    &__item{
      ...
      .btn{
        ...
        &-watch{
          ...
          background-image: url('/img/homeMenu/btn-watch.svg');
        }
      }
      .img{
