  ...
  <div class="popup-invite">
    <div class="popup-invite__header">
      <div class="popup-invite__header-top">
        ...
        <span class="title">{{loc('phone:group:invite:2')}}</span>
        <p class="desc">{{loc('phone:group:invite:3')}}</p>
      </div>
    ...
    >
      <span class="text">{{loc('phone:group:invite:4')}}</span>
...
export default {
  ...
  methods: {
    ...
    toInvite: function(items) {
      const contactsAccountsIds = items.map(item => item.AccountId);
      window.mp.triggerServer('phone::msg::manage::inviteToChat', this.messengerData.CurrentChat.Id, JSON.stringify(contactsAccountsIds));
      this.$emit('close');
    }
  },
